...
import { COGNITION_CONFIG_KEYS } from '../runtimeConfigKeys';

export interface CognitionSettingsSectionProps {
  t: (key: string) => string;
  aiConfig: {
    id: string;
...
export function CognitionSettingsSection({
  t,
  aiConfig,
  onUpdateConfig,
}: CognitionSettingsSectionProps) {
  const entries = COGNITION_CONFIG_KEYS.flatMap((key) => {
    ...
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold flex items-center gap-2 mb-1">
          ...
          {t('aiConfig.cognitionSettings.title')}
        </h2>
        <p className="text-sm text-muted-foreground">
          {t('aiConfig.cognitionSettings.description')}
        </p>
        <Button variant="outline" size="sm" className="mt-3 h-9" asChild>
          <Link to="/ai-runtime?tab=cognition">
            {t('aiConfig.cognitionSettings.openRuntime')}
          </Link>
        </Button>
      </div>
      ...
      {entries.length === 0 ? (
        <div className="text-sm text-muted-foreground p-4 rounded-lg border border-border/30 bg-muted/20">
          {t('aiConfig.cognitionSettings.empty')}
        </div>
      ) : (
        <>
          ...
          <p className="text-xs text-muted-foreground">
            {t('aiConfig.cognitionSettings.prefetchHint')}
          </p>
        </>
      )}
    </div>
